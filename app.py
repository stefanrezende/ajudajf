...
def api_pontos():
    with get_db() as conn:
        rows = conn.execute("SELECT * FROM pontos WHERE ativo=TRUE ORDER BY criado_em").fetchall()
...
def admin_lista():
    with get_db() as conn:
        rows = conn.execute("SELECT * FROM pontos ORDER BY criado_em DESC").fetchall()
    return jsonify([dict(r) for r in rows])
...
def admin_toggle(pid):
    with get_db() as conn:
        conn.execute("UPDATE pontos SET ativo = NOT ativo WHERE id=%s", (pid,))
    return jsonify({"ok": True})
