...
def api_login():
    ...
    if ADMIN_PASSWORD and data.get("senha") == ADMIN_PASSWORD:
        session["admin"] = True
...
@app.route("/api/admin/pontos/<pid>", methods=["PUT"])
@login_required
def admin_editar(pid):
    ...
    with get_db() as conn:
        ensure_table(conn)
        conn.execute("""
            UPDATE pontos SET nome=%s, endereco=%s, latitude=%s, longitude=%s,
              responsavel=%s, telefone=%s, horario=%s, itens=%s
            WHERE id=%s
        """, (d["nome"], d["endereco"], lat, lon,
              d.get("responsavel",""), d.get("telefone",""),
              d.get("horario",""), d.get("itens",""), pid))
    return jsonify({"ok": True})
